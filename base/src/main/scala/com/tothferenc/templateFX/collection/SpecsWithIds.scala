...
final case class SpecsWithIds[Key: ClassTag, Item](specs: immutable.Seq[(Key, Template[Item])]) extends CollectionSpec[Item] {
  ...
  override def requiredChangesIn(collection: JList[Item]): List[Change] = {
    ...
    val mutationsInsertions: immutable.Seq[Change] = specs.flatMap {
